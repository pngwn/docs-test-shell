...
export default function get_sections() {
	...
	return fs
		...
		.map(file => {
			const markdown = fs.readFileSync(`${process.cwd()}/docs/api/${file}`, 'utf-8');
			...
			renderer.heading = (text, level, rawtext) => {
				...
				if (level === 3 || level === 4) {
					...
					section_stack[section_stack.length-1].push({ slug, title, sections: [] });
					prev_level = level;
				}
...
console.log(JSON.stringify(get_sections(), null, 2))